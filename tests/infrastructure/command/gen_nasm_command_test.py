...
'''
Module
    gen_nasm_command_test.py
Info
    Unit tests for GenNasmCommandDefinition and GenNasmCommandExecutor.
'''
...
class TestGenNasmCommand(unittest.TestCase):

    def test_definition(self) -> None:
        definition = GenNasmCommandDefinition()
        self.assertEqual(definition.name, 'create')
        self.assertEqual(definition.help_text, 'Generate Nasm project skeleton')
    ...
    def test_executor_execute_success(self) -> None:
        definition = GenNasmCommandDefinition()
        executor = GenNasmCommandExecutor(definition)
